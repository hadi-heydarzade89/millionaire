...
@section('content')
    <div class="d-flex flex-row-reverse mb-3" style="align-items: center">
        <a href="{{route('admin.questions.create')}}" title="{{__('Add new question')}}">
        ...
        <a href="{{route('admin.questions.create')}}">
            <div style="margin-right: 1.5rem">{{__('Add new question')}}</div>
        </a>
    </div>
    @if($questions->count() === 0)
        <p>{{__('The games list is empty. We will add some question in future.')}}</p>
    @else
        <div class="list-group w-auto">
            @foreach($questions as $question)
                <div class="d-flex gap-2 w-100 justify-content-between">
                    ...
                    <small
                        class="opacity-50 text-nowrap">{{\Carbon\Carbon::parse($question->created_at)->diffForHumans()}}</small>
